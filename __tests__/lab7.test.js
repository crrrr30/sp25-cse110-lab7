...
describe("Basic user flow for Website", () => {
  ...
  it("Make sure <product-item> elements are populated", async () => {
    ...
    if (prodItemsData.length === 0) allArePopulated = false;

    prodItemsData.forEach(({ title, price, image }, i) => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
     ...
     */
    const productItemPromise = page.$("product-item");
    if (!productItemPromise) return;
    const productItem = await productItemPromise;
    ...
    // find the button inside the shadow DOM and click it
    const button = await shadowRoot.$("button");
    if (!button) return;
    await button.click();

    // verify the button text changed to "Remove from Cart"
  ...
  it("Checking number of items in cart on screen", async () => {
    ...
    // First reset the cart to ensure consistent state
    await page.evaluate(() => {
    ...
    for (let i = 0; i < items.length; i++) {
      const item = items[i];
      console.log(`Adding item ${i}/${items.length} to cart`);
  ...
  it("Checking number of items in cart on screen after reload", async () => {
    ...
    for (let i = 0; i < items.length; i++) {
      const item = items[i];
      console.log(`Checking item ${i}/${items.length}`);
      const shadowRootHandle = await item.getProperty("shadowRoot");
  ...
  it("Checking number of items in cart on screen after removing from cart", async () => {
    ...
    for (let i = 0; i < items.length; i++) {
      const item = items[i];
      console.log(`Checking item ${i}/${items.length} after removing from cart`);
      const shadowRootHandle = await item.getProperty("shadowRoot");
  ...
  it("Checking number of items in cart on screen after reload", async () => {
    ...
    for (let i = 0; i < items.length; i++) {
      const item = items[i];
      console.log(`Checking item ${i}/${items.length} from cart`);
      const shadowRootHandle = await item.getProperty("shadowRoot");
